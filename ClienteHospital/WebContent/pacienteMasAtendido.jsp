...
<body>
	...
	%>
		<h1>Paciente Más atendido</h1>
	</div>
	<fieldset>
		<legend>Búsqueda</legend>
		<form action="PacienteMasAtendido" method="post">			

			<input type="submit" value="Calcular porcentaje">
		</form>
	</fieldset>
	<fieldset>
		<legend>Datos paciente más atendido</legend>
		<p hidden>
			<label>Id:</label><input type="text" value="<%if(idPaciente==null){}else{%><%=idPaciente%><%} %>"/>
		</p>
		<p>
			<label>Nombre:</label><input disabled type="text" value="<%if(nombre==null){}else{%><%=nombre%><%} %>">
		</p>
